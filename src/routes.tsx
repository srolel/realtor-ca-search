...
const getRoute = (p: Promise<any>) => p.then(mod => mod.default);
...
routes = [{
  ...
  route: '/favorites',
  async getComponent(appState, params) {
    const Favorites = await getRoute(System.import('./components/Favorites'));
    return <Favorites appState={appState} />;
  }
}];
